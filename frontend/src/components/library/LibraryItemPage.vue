...
import { ArrowLeft, Download, Eye, FileSearch, ListTree, Pencil, ScanText } from "lucide-vue-next"
...
// load 协调当前组件的状态和交互。
async function load(targetId=props.itemId){const targetItemId=targetId;const sequence=++loadSequence;clearSaveTimer();hydratingContent=true;item.value=null;content.value="";officePreview.value=null;dirty.value=false;version.value=0;try{const loaded=await api.getLibraryItem(targetItemId);if(sequence!==loadSequence)return;item.value=loaded;rememberLibraryPath(loaded.parent_id);tagInput.value=(loaded.tags||[]).join(", ");if(loaded.kind==="note"){const data=await api.getLibraryContent(targetItemId);if(sequence!==loadSequence)return;content.value=data.content;version.value=loaded.current_version||data.version;await nextTick();if(sequence!==loadSequence)return}else if(["application/vnd.openxmlformats-officedocument.wordprocessingml.document","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","application/vnd.openxmlformats-officedocument.presentationml.presentation"].includes(loaded.mime_type)){officeLoading.value=true;try{const preview=await api.getLibraryPreview(targetItemId);if(sequence===loadSequence)officePreview.value=preview}catch(error){if(sequence===loadSequence)toast.warning(error.message||"文档预览不可用")}finally{if(sequence===loadSequence)officeLoading.value=false}}dirty.value=false}catch(e){if(sequence===loadSequence)toast.error(e.message||"资料加载失败")}finally{if(sequence===loadSequence)hydratingContent=false}}
...
// ocr 协调当前组件的状态和交互。
async function ocr(event){const file=event.target.files?.[0];if(!file)return;const targetItemId=props.itemId;const targetSequence=loadSequence;try{const queued=await api.ocrImage(file);toast.info("OCR 已加入任务队列，识别完成后会自动插入");const result=await api.waitForOCRTask(queued.task?.id||queued.id);if(targetSequence!==loadSequence||String(props.itemId)!==String(targetItemId)){toast.info("OCR 已完成；你已切换笔记，结果可在任务中心查看");return}content.value += `${content.value?'\n\n':''}${result.result_markdown||result.markdown||result.text||''}`;toast.success("OCR 内容已插入笔记")}catch(e){if(targetSequence===loadSequence)toast.error(e.message||"OCR 失败")}finally{event.target.value=''}}
...
// confirmPendingChanges 在当前界面组件中完成交互或数据处理。
