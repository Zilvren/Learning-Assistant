...
- 注册流程支持邮箱验证；配置 SMTP 后，新用户需完成邮件中的验证链接才能登录。
- 仪表盘新增全年学习记录热力图，支持横向浏览全年、切换年份和查看每天的学习活动。

### Changed

- 产品名称统一为“学习追踪器”，学习记录默认将今天定位在时间轴可视区域右侧，便于向左查看历史。
- 资料库和文件夹卡片在数据加载完成后依次轻微上浮、淡入显示。

### Fixed

- 修复 PostgreSQL 模式导入 ZIP 备份时，已有文件夹层级可能阻止资料库索引、笔记版本和附件恢复的问题。
- 修复回收站会平铺展示已删除文件夹内所有后代文件的问题；现在仅显示删除的顶层项目，恢复或永久删除文件夹会作用于其全部内容。
