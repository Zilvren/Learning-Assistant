...
- AI 页面可选择资料库路径；选中后只会索引该路径及其下的可读文件。资料库内勾选项目后，可通过“发送给 AI 助手”把它们作为本次对话的明确资料范围。
- 勾选一篇 Markdown 或纯文本笔记后，可在 AI 输入框点击铅笔图标进入编辑模式。AI 只会生成完整的修改预览；你确认后才会写回笔记，并保留原版本以便恢复。
- 也可以直接说“帮我写在 `daily/Note/今日整理` 中”。AI 会解析资料库路径：目标不存在时预览创建 Markdown 笔记，目标已存在时预览新版本；任何写入都需要你确认。
- `deepseek-v4-flash` 和 `deepseek-v4-pro` 均支持 1M 上下文。每段对话会独立保留会话记忆；接近安全输入预算时，应用会自动将较早消息整理为摘要，并保留最新 32 条消息继续对话。
- AI 对话会自动保存最新 16 轮用户与助手消息；刷新或重新打开页面后会恢复，并可通过“新对话”清除。
- 单次回答可使用模型最高 384K tokens 的输出上限；若模型达到输出上限，页面会明确提示并提供“继续生成”，避免把半截回答误认为完整内容。
